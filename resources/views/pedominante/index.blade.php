...
@section('content')
    <div class="content-header">
        <h1>{!! trans('messages.t_pedominante') !!}</h1>
    </div>
    <div class="content">
        <div class="row">
        ...
        <div class="row" >
            <table class="display" id="table2">
                <thead>
                <tr>
                    <th>{!! trans('messages.tit_codigo') !!}</th>
                    <th>{!! trans('messages.tit_pedominante') !!}</th>
                </tr>
                </thead>
            </table>
        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            oTable = $('#table2').DataTable({
                ...
                "ordering": false,
                "paging": true,
                "ajax": "{{ asset( 'pedominante/_json' ) }}",
                bAutoWidth:true,
                columns: [
                    {"data": "PE_DOMINANTE"},
                    {"data": "PE_DOMINANTE_DESCRICAO"}
                ],
                dom: 'Bfrtip',
                buttons: [
            ...
            $('#inc').on('click', function () {
                location.href="{!! asset('pedominante/create') !!}";
            });
            ...
            $('#alt').on('click', function() {
                ...
                if ($_data == null) {
                    ...
                    $_id = $_data['PE_DOMINANTE'];
                    url = '{{ asset('pedominante/edit')  }}/' + $_id;
                    location.href=url;
                }
            });
            ...
            $('#exc').on('click', function() {
                ...
                if ($_data == null) {
                    ...
                    var _descr = $_data['PE_DOMINANTE_DESCRICAO'];
                    bootbox.dialog({
                        title  : "{!! trans('messages.exclusao') !!}",
                        message: "{!! trans('messages.exc_pedominante') !!}" + _descr + "?",
                        buttons:{
                            yes: {
                                label:"Sim",
                                ...
                                callback: function() {
                                    // pega o código da cidade
                                    var $_id = $_data['PE_DOMINANTE'];
                                    var url = '{{ asset('pedominante/destroy')  }}/' + $_id;
                                    //location.href=url;
                                    //alert(url);
                                    $.ajax({
                                        type: "get",
