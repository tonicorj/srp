...
@section('content')
    <div class="content-header">
        <h1>{!! trans('messages.t_parceiros') !!}</h1>
    </div>
    <div class="content">
        <div class="row">
        ...
        <div class="row" >
            <table class="display" id="table2">
                <thead>
                <tr>
                    <th>{!! trans('messages.tit_parceiro') !!}</th>
                    <th>{!! trans('messages.tit_contato') !!}</th>
                    <th>{!! trans('messages.tit_telefone') !!}</th>
                    <th>{!! trans('messages.tit_celular') !!}</th>
                </tr>
                </thead>
            </table>
        </div>
        ...
        <script>
            ...
            $(document).ready(function() {
                oTable = $('#table2').DataTable({
                    ...
                    "ordering": false,
                    "paging": true,
                    "ajax": "{{ asset( 'parceiros/_json' ) }}",
                    columns: [
                        {"data": "PARCEIRO_NOME"},
                        {"data": "NOME_CONTATO_PARCEIRO"},
                        {"data": "PARCEIRO_TELEFONE"},
                        {"data": "PARCEIRO_CELULAR"},
                        {"data": "ID_PARCEIRO"}
                    ],
                    dom: 'Bfrtip',
                    buttons: [
                ...
                $('#inc').on('click', function () {
                    location.href="{!! asset('parceiros/create') !!}";
                });
                ...
                $('#alt').on('click', function() {
                    ...
                    if ($_data == null) {
                        ...
                        $_id = $_data['ID_PARCEIRO'];
                        url = '{{ asset('parceiros/edit')  }}/' + $_id;
                        location.href=url;
                    }
                });
                ...
                $('#exc').on('click', function() {
                    ...
                    if ($_data == null) {
                        ...
                        var _descr = $_data['PARCEIRO_NOME'];
                        bootbox.dialog({
                            title  : "{!! trans('messages.exclusao') !!}",
                            message: "{!! trans('messages.exc_parceiro') !!}" + _descr + "?",
                            buttons:{
                                yes: {
                                    label:"Sim",
                                    ...
                                    callback: function() {
                                        // pega o código da cidade
                                        var $_id = $_data['ID_PARCEIRO'];
                                        var url = '{{ asset('parceiros/destroy')  }}/' + $_id;
                                        //location.href=url;
                                        //alert(url);
                                        $.ajax({
                                            type: "get",
