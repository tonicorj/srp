...
@section('content')
    <div class="content-header">
        <h1>{!! trans('messages.t_localatividade') !!}</h1>
    </div>
    <div class="content">
        <div class="row">
        ...
        <div class="row" >
            <table class="display" id="table2">
                <thead>
                <tr>
                    <th>{!! trans('messages.tit_localatividade') !!}</th>
                </tr>
                </thead>
            </table>
        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            oTable = $('#table2').DataTable({
                ...
                "ordering": false,
                "paging": true,
                "ajax": "{{ asset( 'localatividade/_json' ) }}",
                bAutoWidth:true,
                columns: [
                    {"data": "LOCAL_ATIVIDADE_DESCRICAO"},
                    {"data": "ID_LOCAL_ATIVIDADE"}
                ],
                dom: 'Bfrtip',
                buttons: [
            ...
            $('#inc').on('click', function () {
                location.href="{!! asset('localatividade/create') !!}";
            });
            ...
            $('#alt').on('click', function() {
                ...
                if ($_data == null) {
                    ...
                    $_id = $_data['ID_LOCAL_ATIVIDADE'];
                    url = '{{ asset('localatividade/edit')  }}/' + $_id;
                    location.href=url;
                }
            });
            ...
            $('#exc').on('click', function() {
                ...
                if ($_data == null) {
                    ...
                    var _descr = $_data['LOCAL_ATIVIDADE_DESCRICAO'];
                    bootbox.dialog({
                        ...
                        buttons:{
                            yes: {
                                ...
                                callback: function() {
                                    // pega o código da cidade
                                    var $_id = $_data['ID_LOCAL_ATIVIDADE'];
                                    var url = '{{ asset('localtividade/destroy')  }}/' + $_id;
                                    //location.href=url;
                                    //alert(url);
                                    $.ajax({
                                        type: "get",
