...
@section('content')
    <div class="content-header">
        <h1>{!! trans('messages.t_condicaotempo') !!}</h1>
    </div>
    <div class="content">
        <div class="row">
        ...
        <div class="row" >
            <table class="display" id="table2">
                <thead>
                <tr>
                    <th>{!! trans('messages.tit_condicaotempo') !!}</th>
                </tr>
                </thead>
            </table>
        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            oTable = $('#table2').DataTable({
                ...
                "ordering": false,
                "paging": true,
                "ajax": "{{ asset( 'condicaotempo/_json' ) }}",
                bAutoWidth:true,
                columns: [
                    {"data": "CONDICAO_TEMPO"},
                    {"data": "ID_CONDICAO_TEMPO"}
                ],
                dom: 'Bfrtip',
                buttons: [
            ...
            $('#inc').on('click', function () {
                location.href="{!! asset('condicaotempo/create') !!}";
            });
            ...
            $('#alt').on('click', function() {
                ...
                if ($_data == null) {
                    ...
                    $_id = $_data['CONDICAO_TEMPO'];
                    url = '{{ asset('condicaotempo/edit')  }}/' + $_id;
                    location.href=url;
                }
            });
            ...
            $('#exc').on('click', function() {
                ...
                if ($_data == null) {
                    ...
                    var _descr = $_data['CONDICAO_TEMPO'];
                    bootbox.dialog({
                        title  : "{!! trans('messages.exclusao') !!}",
                        message: "{!! trans('messages.exc_condicaotempo') !!}" + _descr + "?",
                        buttons:{
                            yes: {
                                label:"Sim",
                                ...
                                callback: function() {
                                    // pega o código da cidade
                                    var $_id = $_data['CONDICAO_TEMPO'];
                                    var url = '{{ asset('condicaotempo/destroy')  }}/' + $_id;
                                    //location.href=url;
                                    //alert(url);
                                    $.ajax({
                                        type: "get",
