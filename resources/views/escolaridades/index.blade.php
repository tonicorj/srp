...
@section('content')
    <div class="content-header">
        <h1>{!! trans('messages.t_escolaridades') !!}</h1>
    </div>
    <div class="content">
        <div class="row">
        ...
        <div class="row" >
            <table class="display" id="table2">
                <thead>
                <tr>
                    <th>{!! trans('messages.tit_escolaridades') !!}</th>
                </tr>
                </thead>
            </table>
        </div>
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                oTable = $('#table2').DataTable({
                    ...
                    "ordering": false,
                    "paging": true,
                    "ajax": "{{ asset( 'escolaridades/_json' ) }}",
                    bAutoWidth:true,
                    columns: [
                        {"data": "ESCOLARIDADE_DESCRICAO"},
                        {"data": "ID_ESCOLARIDADE"}
                    ],
                    dom: 'Bfrtip',
                    buttons: [
                        {   "text": 'Inclusão',
                ...
                $('#inc').on('click', function () {
                    location.href="{!! asset('escolaridades/create') !!}";
                });
                ...
                $('#alt').on('click', function() {
                    ...
                    if ($_data == null) {
                        ...
                        $_id = $_data['ID_ESCOLARIDADE'];
                        url = '{{ asset('escolaridades/edit')  }}/' + $_id;
                        location.href=url;
                    }
                });
                ...
                $('#exc').on('click', function() {
                    ...
                    if ($_data == null) {
                        ...
                        bootbox.alert('{!! trans('messages.tit_selecione') !!}');
                    }
                    else {
                        var _descr = $_data['ESCOLARIDADE_DESCRICAO'];
                        bootbox.dialog({
                            title  : "{!! trans('messages.exclusao') !!}",
                            message: {!! trans('messages.exc_escolaridades') !!} + _descr + "?",
                            buttons:{
                                yes: {
                                    label:"Sim",
                                    ...
                                    callback: function() {
                                        // pega o código da cidade
                                        var $_id = $_data['ID_ESCOLARIDADE'];
                                        var url = '{{ asset('escolaridades/destroy')  }}/' + $_id;
                                        //location.href=url;
                                        //alert(url);
                                        $.ajax({
                                            type: "get",
