        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <table class='table table-striped' id="tbl_">
                    ...
                    <tbody>
                        @foreach ($juizes as $reg)
                            <tr>
                                {!! Form::open(['route' => [ 'juizes.destroy' ,'id' => $reg->ID_JUIZ]
                                , 'method' =>'DELETE'
                                , 'id' => "delete-form-{$reg->ID_JUIZ}"
                                , 'style' => 'display:none'
                                ]) !!}
                                {{ Form::close() }}
                                <td>{{$reg->ID_JUIZ}}</td>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('jogos/juizes/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('jogos/juizes')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                bootbox.dialog({
                                    ...
                                    message: "{!! trans('messages.exc_juiz') !!}" + _descr + "?",
                                    buttons: {
                                        yes: {
                                            label: "Sim",
