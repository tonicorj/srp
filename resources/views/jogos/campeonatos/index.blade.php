    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <table class='table table-striped' id="tbl_">
                ...
                <tbody>
                @foreach ($campeonatos as $reg)
                    <tr>
                        {!! Form::open(['route' => [ 'campeonatos.destroy' ,'id' => $reg->ID_CAMPEONATO]
                        , 'method' =>'DELETE'
                        , 'id' => "delete-form-{$reg->ID_CAMPEONATO}"
                        , 'style' => 'display:none'
                        ]) !!}
                        {{ Form::close() }}
                        <td>{{$reg->ID_CAMPEONATO}}</td>
                        <td>{{$reg->tipo_campeonato->TCAMP_DESCRICAO}}</td>
                        <td>{{$reg->CAMP_ANO}}</td>
                        <td>{{$reg->categoria->CATEG_DESCRICAO}}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('jogos/campeonatos/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('jogos/campeonatos')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                bootbox.dialog({
                                    ...
                                    message: "{!! trans('messages.exc_campeonato') !!}" + _descr + "?",
                                    buttons: {
                                        yes: {
                                            label: "Sim",
