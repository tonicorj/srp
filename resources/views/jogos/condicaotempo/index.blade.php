        ...
        <div class="panel-body">
            <table class='table table-striped' id="tbl_">
                ...
                @foreach ($condicoes as $reg)
                    <tr>
                        {!! Form::open(['route' => [ 'condicaotempo.destroy' ,'id' => $reg->ID_CONDICAO_TEMPO]
                        , 'method' =>'DELETE'
                        , 'id' => "delete-form-{$reg->ID_CONDICAO_TEMPO}"
                        , 'style' => 'display:none'
                        ]) !!}
                        {{ Form::close() }}
                        <td>{{$reg->ID_CONDICAO_TEMPO}}</td>
                        <td>{{$reg->CONDICAO_TEMPO}}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('jogos/condicaotempo/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('jogos/condicaotempo')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
