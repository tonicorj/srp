    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <table class='table table-striped' id="tbl_">
                ...
                <tbody>
                @foreach ($escopos as $reg)
                    <tr>
                        {!! Form::open(['route' => [ 'escopos.destroy' ,'id' => $reg->ID_ESCOPO]
                        , 'method' =>'DELETE'
                        , 'id' => "delete-form-{$reg->ID_ESCOPO}"
                        , 'style' => 'display:none'
                        ]) !!}
                        {{ Form::close() }}
                        <td>{{$reg->ID_ESCOPO}}</td>
                        <td>{{$reg->ESCOPO_DESCRICAO}}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('jogos/escopos/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('jogos/escopos')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
