        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <table class='table table-striped' id="tbl_">
                    ...
                    <tbody>
                    @foreach ($tipocampeonato as $reg)
                        <tr>
                            {!! Form::open(['route' => [ 'tipocampeonato.destroy' ,'id' => $reg->ID_TIPOCAMP]
                            , 'method' =>'DELETE'
                            , 'id' => "delete-form-{$reg->ID_TIPOCAMP}"
                            , 'style' => 'display:none'
                            ]) !!}
                            {!! Form::close() !!}
                            <td>{{$reg->ID_TIPOCAMP}}</td>
                            <td>{{$reg->TCAMP_DESCRICAO}}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('jogos/tipocampeonato/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('jogos/tipocampeonato')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                bootbox.dialog({
                                    ...
                                    message: "{!! trans('messages.exc_tipocampeonato') !!}" + _descr + "?",
                                    buttons: {
                                        yes: {
                                            label: "Sim",
