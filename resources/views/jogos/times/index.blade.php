        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <table class='table table-striped' id="tbl_">
                    ...
                    <tbody>
                        @foreach ($times as $reg)
                            <tr>
                                {!! Form::open(['route' => [ 'times.destroy' ,'id' => $reg->ID_TIME]
                                , 'method' =>'DELETE'
                                , 'id' => "delete-form-{$reg->ID_TIME}"
                                , 'style' => 'display:none'
                                ]) !!}
                                {{ Form::close() }}
                                <td>{{$reg->ID_TIME}}</td>
                                <td>{{$reg->TIME_NOME}}</td>
                                <td>{{$reg->TIME_NOMEREAL}}</td>
                                <td>{{$reg->UF}}</td>
                                <td>{{$reg->PAIS_NOME}}</td>
                            </tr>
                        @endforeach
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('jogos/times/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                bootbox.dialog({
                                    ...
                                    message: "{!! trans('messages.exc_time') !!}" + _descr + "?",
                                    buttons: {
                                        yes: {
                                            label: "Sim",
