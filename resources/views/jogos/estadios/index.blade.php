    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <table class='table table-striped' id="tbl_">
                ...
                <tbody>
                    @foreach ($estadios as $reg)
                        <tr>
                            {!! Form::open(['route' => [ 'estadios.destroy' ,'id' => $reg->ID_ESTADIO]
                            , 'method' =>'DELETE'
                            , 'id' => "delete-form-{$reg->ID_ESTADIO}"
                            , 'style' => 'display:none'
                            ]) !!}
                            {{ Form::close() }}
                            <td>{{$reg->ID_ESTADIO}}</td>
                            <td>{{$reg->ESTADIO_NOME}}</td>
                            <td>{{$reg->UF}}</td>
                            <td>{{isset($reg->cidades)?$reg->cidades->CIDADE_NOME:'' }}</td>
                            <td>{{isset($reg->paises)?$reg->paises->PAIS_NOME:'' }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('jogos/estadios/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('jogos/estadios')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                bootbox.dialog({
                                    ...
                                    message: "{!! trans('messages.exc_estadio') !!}" + _descr + "?",
                                    buttons: {
                                        yes: {
                                            label: "Sim",
