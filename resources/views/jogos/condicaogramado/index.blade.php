    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <table class='table table-striped' id="tbl_">
                ...
                <tbody>
                @foreach ($condicoes as $reg)
                    <tr>
                        {!! Form::open(['route' => [ 'condicaogramado.destroy' ,'id' => $reg->ID_CONDICAO_GRAMADO]
                        , 'method' =>'DELETE'
                        , 'id' => "delete-form-{$reg->ID_CONDICAO_GRAMADO}"
                        , 'style' => 'display:none'
                        ]) !!}
                        {{ Form::close() }}
                        <td>{{$reg->ID_CONDICAO_GRAMADO}}</td>
                        <td>{{$reg->CONDICAO_GRAMADO}}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('jogos/condicaogramado/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('jogos/condicaogramado')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                bootbox.dialog({
                                    ...
                                    message: "{!! trans('messages.exc_condicaogramado') !!}" + _descr + "?",
                                    buttons: {
                                        yes: {
                                            label: "Sim",
