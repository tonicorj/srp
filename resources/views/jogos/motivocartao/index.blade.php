    ...
    <div class="col-lg-8">
        <div class="panel panel-default">
            <div class="panel-heading">
                <h4>{!! trans('messages.t_motivocartao') !!}</h4>
            </div>
            <div class="panel-body">
                <table class='table table-striped' id="tbl_">
                    ...
                    @foreach ($motivos as $reg)
                        <tr>
                            {!! Form::open(['route' => [ 'motivocartao.destroy' ,'id' => $reg->ID_MOTIVO_CARTAO]
                            , 'method' =>'DELETE'
                            , 'id' => "delete-form-{$reg->ID_MOTIVO_CARTAO}"
                            , 'style' => 'display:none'
                            ]) !!}
                            {!! Form::close() !!}
                            <td>{{$reg->ID_MOTIVO_CARTAO}}</td>
                            <td>{{$reg->MOTIVO_CARTAO}}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('jogos/motivocartao/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('jogos/motivocartao')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                bootbox.dialog({
                                    ...
                                    message: "{!! trans('messages.exc_motivocartao') !!}" + _descr + "?",
                                    buttons: {
                                        yes: {
                                            label: "Sim",
