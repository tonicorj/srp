        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <table class='table table-striped' id="tbl_">
                    ...
                    <tbody>
                    @foreach ($criterios as $reg)
                        <tr>
                            {!! Form::open(['route' => [ 'criterios.destroy' ,'id' => $reg->ID_CRITERIO]
                            , 'method' =>'DELETE'
                            , 'id' => "delete-form-{$reg->ID_CRITERIO}"
                            , 'style' => 'display:none'
                            ]) !!}
                            {!! Form::close() !!}
                            <td>{{$reg->ID_CRITERIO}}</td>
                            <td>{{$reg->CRIT_DESCRICAO}}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
                ...
                buttons: [
                    /*
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('jogos/criterios/create') !!}";
                        }
                    },
                     */
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('jogos/criterios')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                bootbox.dialog({
                                    ...
                                    message: "{!! trans('messages.exc_criterios') !!}" + _descr + "?",
                                    buttons: {
                                        yes: {
                                            label: "Sim",
