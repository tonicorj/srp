    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <table class='table table-striped' id="tbl_">
                ...
                <tbody>
                @foreach ($contas as $reg)
                    <tr>
                        {!! Form::open(['route' => [ 'contas.destroy' ,'id' => $reg->CONTA_ID]
                        , 'method' =>'DELETE'
                        , 'id' => "delete-form-{$reg->CONTA_ID}"
                        , 'style' => 'display:none'
                        ]) !!}
                        {!! Form::close() !!}
                        <td>{{$reg->CONTA_ID}}</td>
                        <td>{{$reg->tipo_conta->TIPO_CONTA_DESCRICAO}}</td>
                        <td>{{$reg->CONTA_NUMERO}}</td>
                        <td>{{$reg->CONTA_NOME}}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('financeiro/contas/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('financeiro/contas')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                bootbox.dialog({
                                    ...
                                    message: "{!! trans('messages.exc_conta') !!}" + _descr + "?",
                                    buttons: {
                                        yes: {
                                            label: "Sim",
