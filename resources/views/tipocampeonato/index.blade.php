...
@section('content')
    <div class="content-header">
        <h1>{!! trans('messages.t_tipocampeonato') !!}</h1>
    </div>
    <div class="content">
        <div class="row">
        ...
        <div class="row" >
            <table class="display" id="table2">
                <thead>
                <tr>
                    <th>{!! trans('messages.tit_tipocampeonato') !!}</th>
                    <th>{!! trans('messages.tit_escopo') !!}</th>
                </tr>
                </thead>
            </table>
        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            oTable = $('#table2').DataTable({
                ...
                "ordering": false,
                "paging": true,
                "ajax": "{{ asset( 'tipocampeonato/_json' ) }}",
                bAutoWidth:true,
                columns: [
                    {"data": "TCAMP_DESCRICAO"},
                    {"data": "ESCOPO_DESCRICAO"},
                    {"data": "ID_TIPOCAMP"}
                ],
                dom: 'Bfrtip',
                buttons: [
            ...
            $('#inc').on('click', function () {
                location.href="{!! asset('tipocampeonato/create') !!}";
            });
            ...
            $('#alt').on('click', function() {
                ...
                if ($_data == null) {
                    ...
                    $_id = $_data['ID_TIPOCAMP'];
                    url = '{{ asset('tipocampeonato/edit')  }}/' + $_id;
                    location.href=url;
                }
            });
            ...
            $('#exc').on('click', function() {
                ...
                if ($_data == null) {
                    ...
                    var _descr = $_data['TCAMP_DESCRICAO'];
                    bootbox.dialog({
                        title  : "{!! trans('messages.exclusao') !!}",
                        message: "{!! trans('messages.exc_escopo') !!}" + _descr + "?",
                        buttons:{
                            yes: {
                                label:"Sim",
                                ...
                                callback: function() {
                                    // pega o código da cidade
                                    var $_id = $_data['ID_TIPOCAMP'];
                                    var url = '{{ asset('exames/destroy')  }}/' + $_id;
                                    //location.href=url;
                                    //alert(url);
                                    $.ajax({
                                        type: "get",
