...
@section('content')
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <h4>{!! trans('messages.t_dmexame') !!}</h4>
        </div>
        <div class="panel-body content">
            <table class='table table-striped' id="tbl_">
                ...
                <tbody>
                @foreach ($dmexames as $de)
                    <tr>
                        <td>{{$de->id_dm_exame}}</td>
                        <td>{{data_display($de->exame_data)}}</td>
                        <td>{{$de->medico_nome }}</td>
                        <td>{{$de->exame_nome }}</td>
                        <td>{{data_display($de->exame_data_realizado)}}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            location.href = "{!! asset('DM/dmexames/create') !!}" + "?dm=" + id;
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                bootbox.dialog({
                                    ...
                                    message: "{!! trans('messages.exc_exames') !!}" + _descr + "?",
                                    buttons: {
                                        yes: {
                                            label: "Sim",
                                            ...
                                            callback: function () {
                                                // pega o código
                                                var id = dados[0];
                                                var url = '{{ asset('DM/dmexames')  }}/' + $_id + '/destroy';
                                                //location.href=url;
                                                //alert(url);
                                                $.ajax({
                                                    type: "get",
