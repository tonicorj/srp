...
@section('content')
    <div class="content-header">
        <h1>{!! trans('messages.t_motivo_ausencia') !!}</h1>
    </div>
    <div class="content">
        <div class="row">
        ...
        <div class="row" >
            <table class="display" id="table2">
                <thead>
                <tr>
                    <th>{!! trans('messages.t_motivo_ausencia') !!}</th>
                </tr>
                </thead>
            </table>
        </div>
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                oTable = $('#table2').DataTable({
                    ...
                    "ordering": false,
                    "paging": true,
                    "ajax": "{{ asset( 'motivo_ausencia/_json' ) }}",
                    bAutoWidth:true,
                    columns: [
                        {"data": "MOTIVO_AUSENCIA_DESCRICAO"},
                        {"data": "ID_MOTIVO_AUSENCIA"}
                    ],
                    dom: 'Bfrtip',
                    buttons: [
                ...
                $('#inc').on('click', function () {
                    location.href="{!! asset('motivo_ausencia/create') !!}";
                });
                ...
                $('#alt').on('click', function() {
                    ...
                    if ($_data == null) {
                        ...
                        $_id = $_data['ID_MOTIVO_AUSENCIA'];
                        url = '{{ asset('motivo_ausencia/edit')  }}/' + $_id;
                        location.href=url;
                    }
                });
                ...
                $('#exc').on('click', function() {
                    ...
                    if ($_data == null) {
                        ...
                        var _descr = $_data['MOTIVO_AUSENCIA_DESCRICAO'];
                        bootbox.dialog({
                            ...
                            buttons:{
                                yes: {
                                    ...
                                    callback: function() {
                                        // pega o código da cidade
                                        var $_id = $_data['ID_MOTIVO_AUSENCIA'];
                                        var url = '{{ asset('motivo_ausencia/destroy')  }}/' + $_id;
                                        //location.href=url;
                                        //alert(url);
                                        $.ajax({
                                            type: "get",
