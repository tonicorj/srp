...
@section('content')
    <div class="content-header">
        <h1>{!! trans('messages.t_atividadepedagogicas') !!}</h1>
    </div>
    <div class="content">
        <div class="row">
        ...
        <div class="row" >
            <table class="display" id="table2">
                <thead>
                <tr>
                    <th>{!! trans('messages.tit_atividade') !!}</th>
                </tr>
                </thead>
            </table>
        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            oTable = $('#table2').DataTable({
                ...
                "ordering": false,
                "paging": true,
                "ajax": "{{ asset( 'atividadesPedagogicas/_json' ) }}",
                bAutoWidth:true,
                columns: [
                    {"data": "ATIV_PEDAGOGICA_DESCR"},
                    {"data": "ID_ATIV_PEDAGOGICA"}
                ],
                dom: 'Bfrtip',
                buttons: [
            ...
            $('#inc').on('click', function () {
                location.href="{!! asset('atividadesPedagogicas/create') !!}";
            });
            ...
            $('#alt').on('click', function() {
                ...
                if ($_data == null) {
                    ...
                    $_id = $_data['ATIV_PEDAGOGICA_DESCR'];
                    url = '{{ asset('atividadesPedagogicas/edit')  }}/' + $_id;
                    location.href=url;
                }
            });
            ...
            $('#exc').on('click', function() {
                ...
                if ($_data == null) {
                    ...
                    var _descr = $_data['ATIV_PEDAGOGICA_DESCR'];
                    bootbox.dialog({
                        ...
                        buttons:{
                            yes: {
                                ...
                                callback: function() {
                                    // pega o código da cidade
                                    var $_id = $_data['ATIV_PEDAGOGICA_DESCR'];
                                    var url = '{{ asset('atividadesPedagogicas/destroy')  }}/' + $_id;
                                    //location.href=url;
                                    //alert(url);
                                    $.ajax({
                                        type: "get",
