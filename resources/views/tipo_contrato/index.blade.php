...
@section('content')
    <div class="content-header">
        <h1>{!!  trans('messages.t_tipo_contrato') !!}</h1>
    </div>
    <div class="content">
        <div class="row">
        ...
        <div class="row" >
            <table class="display" id="table2">
                <thead>
                <tr>
                    <th>{!!  trans('messages.t_tipo_contrato') !!}</th>
                </tr>
                </thead>
            </table>
        </div>
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                oTable = $('#table2').DataTable({
                    ...
                    "ordering": false,
                    "paging": true,
                    "ajax": "{{ asset( 'tipo_contrato/_json' ) }}",
                    bAutoWidth:true,
                    columns: [
                        {"data": "TIPO_CONTRATO_DESCRICAO"},
                ...
                $('#alt').on('click', function() {
                    ...
                    if ($_data == null) {
                        ...
                        $_id = $_data['ID_TIPO_CONTRATO'];
                        url = '{{ asset('tipo_contrato/edit')  }}/' + $_id;
                        location.href=url;
                    }
                });
                ...
                $('#exc').on('click', function() {
                    ...
                    if ($_data == null) {
                        ...
                        var _descr = $_data['TIPO_CONTRATO_DESCRICAO'];
                        bootbox.dialog({
                            title  : "{!!  trans('messages.exclusao') !!}",
                            message: "{!!  trans('messages.exc_tipocontrato') !!}" + _descr + "?",
                            buttons:{
                                yes: {
                                    label:"Sim",
                                    ...
                                    callback: function() {
                                        // pega o código da cidade
                                        var $_id = $_data['ID_TIPO_CONTRATO'];
                                        var url = '{{ asset('tipo_contrato/destroy')  }}/' + $_id;
                                        //location.href=url;
                                        //alert(url);
                                        $.ajax({
                                            type: "get",
