            ...
            <div class="panel-body">
                <table class='table table-striped' id="tbl_">
                    ...
                    @foreach ($motivos as $reg)
                        <tr>
                            {!! Form::open(['route' => [ 'motivoafastamento.destroy' ,'id' => $reg->ID_MOTIVO_AFASTAMENTO]
                            , 'method' =>'DELETE'
                            , 'id' => "delete-form-{$reg->ID_MOTIVO_AFASTAMENTO}"
                            , 'style' => 'display:none'
                            ]) !!}
                            {!! Form::close() !!}
                            <td>{{$reg->ID_MOTIVO_AFASTAMENTO}}</td>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('adm/motivoafastamento/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('adm/motivoafastamento')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                bootbox.dialog({
                                    ...
                                    message: "{!! trans('messages.exc_motivoAfastamento') !!}" + _descr + "?",
                                    buttons: {
                                        yes: {
                                            label: "Sim",
