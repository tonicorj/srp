    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <table class='table table-striped' id="tbl_">
                ...
                <tbody>
                @foreach ($ocorrencias as $reg)
                    <tr>
                        {!! Form::open(['route' => [ 'ocorrencias.destroy' ,'id' => $reg->ID_JOGADOR_OCORRENCIA]
                        , 'method' =>'DELETE'
                        , 'id' => "delete-form-{$reg->ID_JOGADOR_OCORRENCIA}"
                        , 'style' => 'display:none'
                        ]) !!}
                        {!! Form::close() !!}
                        <td>{{$reg->ID_JOGADOR_OCORRENCIA}}</td>
                        <td>{{ data_display($reg->OCORR_DATA)}}</td>
                        <td>{{$reg->JOG_NOME}}</td>
                        <td>{{$reg->CATEG_DESCRICAO}}</td>
                        <td>{{$reg->DESC_OCORR_TIPO}}</td>
                        <td>{{$reg->OCORR_DESCRICAO}}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('adm/ocorrencias/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('adm/ocorrencias')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                bootbox.dialog({
                                    ...
                                    message: "{!! trans('messages.exc_ocorrencias') !!}" + _descr + "?",
                                    buttons: {
                                        yes: {
                                            label: "Sim",
