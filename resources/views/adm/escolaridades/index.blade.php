...
@section('content')
    <div class="panel panel-default">
        <div class="panel-heading">
            <h4>{!! trans('messages.tit_escolaridades') !!}</h4>
        </div>
        <div class="panel-body">
            <table class='table table-striped' id="tbl_">
                ...
                <tbody>
                @foreach ($escolaridades as $reg)
                    <tr>
                        {!! Form::open(['route' => [ 'escolaridades.destroy' ,'id' => $reg->ID_ESCOLARIDADE]
                        , 'method' =>'DELETE'
                        , 'id' => "delete-form-{$reg->ID_ESCOLARIDADE}"
                        , 'style' => 'display:none'
                        ]) !!}
                        {!! Form::close() !!}
                        <td>{{$reg->ID_ESCOLARIDADE}}</td>
                        <td>{{$reg->ESCOLARIDADE_DESCRICAO}}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('adm/escolaridades/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('adm/escolaridades')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                bootbox.dialog({
                                    ...
                                    message: "{!! trans('messages.exc_escolaridade') !!}" + _descr + "?",
                                    buttons: {
                                        yes: {
                                            label: "Sim",
