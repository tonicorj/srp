        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <table class='table table-striped' id="tbl_">
                    ...
                    <tbody>
                    @foreach ($afastamentos as $reg)
                        <tr>
                            {!! Form::open(['route' => [ 'afastamentos.destroy' ,'id' => $reg->ID_AFASTAMENTO]
                            , 'method' =>'DELETE'
                            , 'id' => "delete-form-{$reg->ID_AFASTAMENTO}"
                            , 'style' => 'display:none'
                            ]) !!}
                            {!! Form::close() !!}
                            <td>{{$reg->ID_AFASTAMENTO}}</td>
                            <td>{{data_display($reg->DATA_SAIDA)}}</td>
                            <td>{{data_display($reg->DATA_RETORNO)}}</td>
                            <td>{{$reg->JOG_NOME_COMPLETO}}</td>
                            <td>{{$reg->MOTIVO_AFASTAMENTO_DESCRICAO}}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('adm/afastamentos/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('adm/afastamentos')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                bootbox.dialog({
                                    ...
                                    message: "{!! trans('messages.exc_afastamento') !!}" + _descr + "?",
                                    buttons: {
                                        yes: {
                                            label: "Sim",
