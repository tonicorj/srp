...
@section('content')
    <div class="panel panel-default">
        <div class="panel-heading">
            <h4>{!! trans('messages.tit_estadocivil') !!}</h4>
        </div>
        <div class="panel-body">
            <table class='table table-striped' id="tbl_">
                ...
                @foreach ($estados as $reg)
                    <tr>
                        {!! Form::open(['route' => [ 'estadocivil.destroy' ,'id' => $reg->ID_ESTADOCIVIL]
                        , 'method' =>'DELETE'
                        , 'id' => "delete-form-{$reg->ID_ESTADOCIVIL}"
                        , 'style' => 'display:none'
                        ]) !!}
                        {!! Form::close() !!}
                        <td>{{$reg->ID_ESTADOCIVIL}}</td>
                        <td>{{$reg->ESTADOCIVIL_DESCRICAO}}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('adm/estadocivil/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('adm/estadocivil')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
