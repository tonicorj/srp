        ...
        <div class="panel-body">
            <table class='table table-striped' id="tbl_">
                ...
                @foreach ($atendimentos as $reg)
                    <tr>
                        {!! Form::open(['route' => [ 'atendimentoSS_func.destroy' ,'id' => $reg->ID_ATEND_ASSIST_SOCIAL]
                        , 'method' =>'DELETE'
                        , 'id' => "delete-form-{$reg->ID_ATEND_ASSIST_SOCIAL}"
                        , 'style' => 'display:none'
                        ]) !!}
                        {{ Form::close() }}
                        <td>{{$reg->ID_ATEND_ASSIST_SOCIAL}}</td>
                        <td>{{data_display($reg->VISITA_DATA)}}</td>
                        <td>{{$reg->NOME}}</td>
                        <td>{{$reg->ATIV_ASSIST_SOCIAL_DESCR}}</td>
                        <td>{{$reg->ORIGEM_SERVSOCIAL_DESCRICAO}}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('ssocial/atendimentoSS_func/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('ssocial/atendimentoSS_func')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                bootbox.dialog({
                                    ...
                                    message: "{!! trans('messages.exc_atendimentoSS_func') !!}" + _descr + "?",
                                    buttons: {
                                        yes: {
                                            label: "Sim",
