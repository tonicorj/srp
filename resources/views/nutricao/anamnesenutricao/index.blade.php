            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <table class='table table-striped' id="tbl_">
                        ...
                        <tbody>
                        @foreach ($anamnese as $reg)
                            <tr>
                                {!! Form::open(['route' => [ 'anamnesenutricao.destroy' ,'id' => $reg->ID_ANAMNESE_NUTRICIONAL]
                                , 'method' =>'DELETE'
                                , 'id' => "delete-form-{$reg->ID_ANAMNESE_NUTRICIONAL}"
                                , 'style' => 'display:none'
                                ]) !!}
                                {{ Form::close() }}
                                <td>{{$reg->ID_ANAMNESE_NUTRICIONAL}}</td>
                                <td>{{data_display($reg->ANAMNESE_DATA)}}</td>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('nutricao/anamnesenutricao/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('nutricao/anamnesenutricao')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                bootbox.dialog({
                                    ...
                                    message: "{!! trans('messages.exc_anamnesenutricao') !!}" + _descr + "?",
                                    buttons: {
                                        yes: {
                                            label: "Sim",
