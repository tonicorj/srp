        ...
        <div class="panel-body">
            <table class='table table-striped' id="tbl_">
                ...
                @foreach ($atividades as $reg)
                    <tr>
                        {!! Form::open(['route' => [ 'atividadesSS.destroy' ,'id' => $reg->ID_ATIV_ASSIST_SOCIAL]
                        , 'method' =>'DELETE'
                        , 'id' => "delete-form-{$reg->ID_ATIV_ASSIST_SOCIAL}"
                        , 'style' => 'display:none'
                        ]) !!}
                        {!! Form::close() !!}
                        <td>{{$reg->ID_ATIV_ASSIST_SOCIAL}}</td>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('SSocial/atividadesSS/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('SSocial/atividadesSS')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
