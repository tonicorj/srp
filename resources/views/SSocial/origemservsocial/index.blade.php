            ...
            <table class='table table-striped' id="tbl_">
                ...
                <tbody>
                    @foreach ($origens as $reg)
                        <tr>
                            {!! Form::open(['route' => [ 'origemservsocial.destroy' ,'id' => $reg->ID_ORIGEM_SERVSOCIAL]
                                , 'method' =>'DELETE'
                                , 'id' => "delete-form-{$reg->ID_ORIGEM_SERVSOCIAL}"
                                , 'style' => 'display:none'
                            ]) !!}
                            {!! Form::close() !!}
                            <td>{{$reg->ID_ORIGEM_SERVSOCIAL}}</td>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('SSocial/origemservsocial/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('SSocial/origemservsocial')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                bootbox.dialog({
                                    ...
                                    message: "{!! trans('messages.exc_origemservsocial_dia') !!}" + _descr + "?",
                                    buttons: {
                                        yes: {
                                            label: "Sim",
