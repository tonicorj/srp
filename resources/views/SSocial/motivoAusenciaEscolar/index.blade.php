        ...
        <div class="panel-body">
            <table class='table table-striped' id="tbl_">
                ...
                @foreach ($motivos as $reg)
                    <tr>
                        {!! Form::open(['route' => [ 'motivoAusenciaEscolar.destroy' ,'id' => $reg->ID_MOTIVO_AUSENCIA_ESCOLAR]
                        , 'method' =>'DELETE'
                        , 'id' => "delete-form-{$reg->ID_MOTIVO_AUSENCIA_ESCOLAR}"
                        , 'style' => 'display:none'
                        ]) !!}
                        {{ Form::close() }}
                        <td>{{$reg->ID_MOTIVO_AUSENCIA_ESCOLAR}}</td>
                ...
                buttons: [
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            location.href = "{!! asset('SSocial/motivoAusenciaEscolar/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function (e, dt, node, config) {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('SSocial/motivoAusenciaEscolar')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
