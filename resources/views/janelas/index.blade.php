...
@section('content')
    <div class="content-header">
        <h1>{!! trans('messages.t_janelas') !!}</h1>
    </div>
    <div class="content">
        <div class="row">
        ...
        <div class="row" >
            <table class="display" id="table2">
                <thead>
                <tr>
                    <th>{!! trans('messages.tit_janela') !!}</th>
                    <th>{!! trans('messages.tit_janela_inicio') !!}</th>
                    <th>{!! trans('messages.tit_janela_final') !!}</th>
                </tr>
                </thead>
            </table>
        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            oTable = $('#table2').DataTable({
                ...
                "ordering": false,
                "paging": true,
                "ajax": "{{ asset( 'janelas/_json' ) }}",
                bAutoWidth:true,
                columns: [
                    {"data": "JANELA_NOME"},
                    {"data": "JANELA_INICIO"},
                    {"data": "JANELA_FINAL"},
                    {"data": "ID_ATIV_PSICOLOGIA"}
                ],
                dom: 'Bfrtip',
                buttons: [
            ...
            $('#inc').on('click', function () {
                location.href="{!! asset('janelas/create') !!}";
            });
            ...
            $('#alt').on('click', function() {
                ...
                if ($_data == null) {
                    ...
                    $_id = $_data['JANELA_NOME'];
                    url = '{{ asset('janelas/edit')  }}/' + $_id;
                    location.href=url;
                }
            });
            ...
            $('#exc').on('click', function() {
                ...
                if ($_data == null) {
                    ...
                    var _descr = $_data['JANELA_NOME'];
                    bootbox.dialog({
                        title  : "{!! trans('messages.exclusao') !!}",
                        message: "{!! trans('messages.exc_janela') !!}" + _descr + "?",
                        buttons:{
                            yes: {
                                label:"Sim",
                                ...
                                callback: function() {
                                    // pega o código da cidade
                                    var $_id = $_data['JANELA_NOME'];
                                    var url = '{{ asset('janela/destroy')  }}/' + $_id;
                                    //location.href=url;
                                    //alert(url);
                                    $.ajax({
                                        type: "get",
