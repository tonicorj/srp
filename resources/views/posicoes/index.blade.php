...
@section('content')
    <div class="content-header">
        <h1>{!! trans('messages.t_posicoes') !!}</h1>
    </div>
    <div class="content">
        <div class="row">
        ...
        <div class="row" >
            <table class="display" id="table2">
                <thead>
                <tr>
                    <th>{!! trans('messages.tit_posicoes') !!}</th>
                </tr>
                </thead>
            </table>
        </div>
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                oTable = $('#table2').DataTable({
                    ...
                    "ordering": false,
                    "paging": true,
                    "ajax": "{{ asset( 'posicoes/_json' ) }}",
                    bAutoWidth:true,
                    columns: [
                        {"data": "POSICAO"},
                        {"data": "POSICAO_DESCRICAO"},
                        {"data": "POSICAO_ORDEM"},
                        {"data": "POSICAO_CAMPO"}
                    ],
                    dom: 'Bfrtip',
                ...
                $('#inc').on('click', function () {
                    location.href="{!! asset('posicoes/create') !!}";
                });
                ...
                $('#exc').on('click', function() {
                    ...
                    if ($_data == null) {
                        ...
                        var _descr = $_data['POSICAO_DESCRICAO'];
                        bootbox.dialog({
                            title  : "{!!  trans('messages.exclusao') !!}",
                            message: "{!!  trans('messages.exc_posicoes') !!}" + _descr + "?",
                            buttons:{
                                yes: {
                                    label:"Sim",
                                    ...
                                    callback: function() {
                                        ...
                                        var $_id = $_data['POSICAO'];
                                        var url = '{{ asset('posicoes/destroy')  }}/' + $_id;
                                        //location.href=url;
                                        //alert(url);
                                        $.ajax({
                                            type: "get",
