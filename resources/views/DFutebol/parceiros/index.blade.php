...
@section('content')
    <div class="panel panel-default">
        <div class="panel-heading">
            <h4>{!! trans('messages.tit_parceiro') !!}</h4>
        </div>
        <div class="panel-body">
            <table class='table table-striped' id="tbl_">
                ...
                <tbody>
                @foreach ($parceiros as $reg)
                    <tr>
                        {!! Form::open(['route' => [ 'parceiros.destroy' ,'id' => $reg->ID_PARCEIRO]
                        , 'method' =>'DELETE'
                        , 'id' => "delete-form-{$reg->ID_PARCEIRO}"
                        , 'style' => 'display:none'
                        ]) !!}
                        {!! Form::close() !!}
                        <td>{{$reg->ID_PARCEIRO}}</td>
                        <td>{{$reg->PARCEIRO_NOME}}</td>
                        <td>{{$reg->PARCEIRO_CELULAR}}</td>
                        <td>{{$reg->NOME_CONTATO_PARCEIRO}}</td>
                        <td>{{$reg->PARCEIRO_MAIL}}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
                ...
                buttons: [
                    {
                        ...
                        "action": function () {
                            location.href = "{!! asset('DFutebol/parceiros/create') !!}";
                        }
                    },
                    {
                        ...
                        "action": function () {
                            ...
                            if (dados == null) {
                                ...
                                // pega o código
                                id = dados[0];
                                url = '{{ asset('DFutebol/parceiros')  }}/' + id + '/edit';
                                location.href = url;
                            }
                        }
                    },
