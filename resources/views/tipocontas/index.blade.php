...
@section('content')
    <div class="content-header">
        <h1>{!! trans('messages.t_tipocontas') !!}</h1>
    </div>
    <div class="content">
        <div class="row">
        ...
        <div class="row" >
            <table class="display" id="table2">
                <thead>
                <tr>
                    <th>{!! trans('messages.tit_tipocontas') !!}</th>
                    <th>{!! trans('messages.tit_tipocontas_tipo') !!}</th>
                    <th>{!! trans('messages.tit_tipocontas_num') !!}</th>
                </tr>
                </thead>
            </table>
        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            oTable = $('#table2').DataTable({
                ...
                "ordering": false,
                "paging": true,
                "ajax": "{{ asset( 'tipocontas/_json' ) }}",
                bAutoWidth:true,
                columns: [
                    {"data": "TIPO_CONTA_DESCRICAO"},
                    {"data": "TIPO_CONTA_TIPO"},
                    {"data": "TIPO_CONTA_NUM"},
                    {"data": "TIPO_CONTA_ID"}
                ],
                dom: 'Bfrtip',
            ...
            $('#inc').on('click', function () {
                location.href="{!! asset('tipocontas/create') !!}";
            });
            ...
            $('#alt').on('click', function() {
                ...
                if ($_data == null) {
                    ...
                    $_id = $_data['TIPO_CONTA_ID'];
                    url = '{{ asset('tipocontas/edit')  }}/' + $_id;
                    location.href=url;
                }
            });
            ...
            $('#exc').on('click', function() {
                ...
                if ($_data == null) {
                    ...
                    var _descr = $_data['TIPO_CONTA_DESCRICAO'];
                    bootbox.dialog({
                        title  : "{!! trans('messages.exclusao') !!}",
                        message: "{!! trans('messages.exc_tipocontas') !!}" + _descr + "?",
                        buttons:{
                            yes: {
                                label:"Sim",
                                ...
                                callback: function() {
                                    // pega o código da cidade
                                    var $_id = $_data['TIPO_CONTA_ID'];
                                    var url = '{{ asset('tipocontas/destroy')  }}/' + $_id;
                                    //location.href=url;
                                    //alert(url);
                                    $.ajax({
                                        type: "get",
