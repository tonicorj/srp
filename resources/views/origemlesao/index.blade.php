...
@section('content')
    <div class="content-header">
        <h1>{!! trans('messages.t_origemlesao') !!}</h1>
    </div>
    <div class="content">
        <div class="row">
        ...
        <div class="row" >
            <table class="display" id="table2">
                <thead>
                <tr>
                    <th>{!! trans('messages.tit_origemlesao') !!}</th>
                </tr>
                </thead>
            </table>
        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            oTable = $('#table2').DataTable({
                ...
                "ordering": false,
                "paging": true,
                "ajax": "{{ asset( 'origemlesao/_json' ) }}",
                bAutoWidth:true,
                columns: [
                    {"data": "ORIGEM_LESAO_DESCRICAO"},
                    {"data": "ID_ORIGEM_LESAO"}
                ],
                dom: 'Bfrtip',
                buttons: [
            ...
            $('#inc').on('click', function () {
                location.href="{!! asset('origemlesao/create') !!}";
            });
            ...
            $('#alt').on('click', function() {
                ...
                if ($_data == null) {
                    ...
                    $_id = $_data['ID_ORIGEM_LESAO'];
                    url = '{{ asset('origemlesao/edit')  }}/' + $_id;
                    location.href=url;
                }
            });
            ...
            $('#exc').on('click', function() {
                ...
                if ($_data == null) {
                    ...
                    var _descr = $_data['ORIGEM_LESAO_DESCRICAO'];
                    bootbox.dialog({
                        title  : "{!! trans('messages.exclusao') !!}",
                        message: "{!! trans('messages.exc_origemlesao') !!}" + _descr + "?",
                        buttons:{
                            yes: {
                                label:"Sim",
                                ...
                                callback: function() {
                                    // pega o código da cidade
                                    var $_id = $_data['ID_ORIGEM_LESAO'];
                                    var url = '{{ asset('origemlesao/destroy')  }}/' + $_id;
                                    //location.href=url;
                                    //alert(url);
                                    $.ajax({
                                        type: "get",
